...
4. **Subqueries come in Phase 6.** A clause such as `field IN (subquery)` requires us to track a second table, because rows can enter and leave the shape when the subquery result changes even though the row itself did not change. Phase 6 implements this for the bare form — the shape's *entire* filter is exactly one `field IN (subquery)` (or `NOT IN`) clause — and for that clause combined with exactly one subquery-free predicate over a top-level `AND` or `OR`: the evaluator decides the rest of the clause against each row a subquery membership flip could affect. More than one subquery in a clause, or a subquery under a top-level `NOT` alongside another predicate, still returns `400`.
...
1. Support `field IN (subquery)`. Track the second table, so rows enter and leave the shape when the subquery result changes even though the row itself did not change. Do this incrementally for the bare form — a shape's entire filter is exactly one `field IN (subquery)` (or `NOT IN`) clause — and for that clause combined with exactly one subquery-free predicate over a top-level `AND` or `OR`, deciding the rest of the clause against each row a subquery membership flip could affect. More than one subquery in a clause, or a subquery under a top-level `NOT` alongside another predicate, remains a `400`.
...
- `field IN (subquery)` and `field NOT IN (subquery)`, as a shape's entire filter or combined with one other predicate over `AND`/`OR`, incrementally track the subquery's table: a change to that table that flips a value's membership produces an `insert` or `delete` for every outer row currently matching that value (and, when combined, still satisfying the rest of the clause), on every shape tracking it. More than one subquery, or a subquery under a top-level `NOT` alongside another predicate, still returns `400` at subscription time.
...
| `where` clauses containing subqueries | Yes | Partly | Phase 6. The bare form (`field IN (subquery)` as the shape's entire filter), and combined with exactly one predicate over a top-level `AND` or `OR`, is incrementally tracked. More than one subquery, or a subquery under a top-level `NOT` alongside another predicate, returns `400`. |
