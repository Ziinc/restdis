defmodule RestdisBuster.Singleton do
  @moduledoc """
  ...
  def notify_ownership_acquired do
    case Application.get_env(:restdis_buster, :failover_reconciler) do
      nil -> :ok
  ...
  def handle_info(:try_register, state) do
    case :syn.register(:wal, :wal_tailer, self()) do
      :ok ->
        :telemetry.execute(
          [:restdis_buster, :singleton, :owner],
          %{is_owner: 1},
          %{node: node()}
        )
        ...
        case start_tailer() do
          {:ok, pid} ->
            Process.monitor(pid)
            Logger.info("[RestdisBuster] This node is the WAL tailer singleton")
            notify_ownership_acquired()
            {:noreply, %{state | tailer: pid}}

          {:error, reason} ->
            Logger.warning(
              "[RestdisBuster] Failed to start Tailer: #{inspect(reason)}, retrying in 2s"
            )
  ...
  def handle_info({:DOWN, _ref, :process, pid, reason}, %{tailer: pid} = state) do
    # Our own Tailer died
    Logger.warning("[RestdisBuster] Tailer crashed (#{inspect(reason)}), restarting")
    :syn.unregister(:wal, :wal_tailer)
  ...
  def handle_info({:DOWN, _ref, :process, _pid, _reason}, state) do
    ...
    Logger.info("[RestdisBuster] WAL tailer singleton went down, attempting takeover")
    send(self(), :try_register)
    {:noreply, state}
  end
  ...
  defp start_tailer do
    DynamicSupervisor.start_child(
      RestdisBuster.TailerSupervisor,
      {RestdisBuster.Tailer, []}
    )
  end
