defmodule RestdisBuster.Worker.Supervisor do
  @moduledoc """
  ...
  Owns two named ETS tables:
    ...
    * `:restdis_buster_tenant_semaphores` — `{tenant_id, counter_ref}`
    * `:restdis_buster_coalesce`          — `{{tenant_id, table}, count}`
  """
  ...
  def start_worker(event, runner \\ &RestdisBuster.Worker.run/1)
  ...
  defp per_tenant_cap do
    Application.get_env(:restdis_buster, :worker_per_tenant_cap, @default_per_tenant_cap)
  end
end
