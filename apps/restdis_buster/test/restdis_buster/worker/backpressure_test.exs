defmodule RestdisBuster.Worker.BackpressureTest do
  ...
  @semaphores_table :restdis_buster_tenant_semaphores
  @coalesce_table :restdis_buster_coalesce
  @tenant "bp_tenant"
  @table "bp_widgets"
  ...
  setup do
    ...
    if sweeper_pid do
      ...
      Supervisor.terminate_child(RestdisBuster.Supervisor, CoalesceSweeper)
    ...
    on_exit(fn ->
      Application.delete_env(:restdis_buster, :worker_per_tenant_cap)
      Restdis.Cache.flush_tenant(@tenant)
      clear_state()
      _ = Supervisor.restart_child(RestdisBuster.Supervisor, CoalesceSweeper)
    end)
  ...
  test "exceeding per-tenant cap coalesces events and emits telemetry" do
    ...
    :telemetry.attach(
      ...
      [:restdis_buster, :backpressure, :triggered],
  ...
  test "infra table events bypass backpressure (always spawn)" do
    Application.put_env(:restdis_buster, :worker_per_tenant_cap, 0)
    on_exit(fn -> Application.put_env(:restdis_buster, :worker_per_tenant_cap, 2) end)
