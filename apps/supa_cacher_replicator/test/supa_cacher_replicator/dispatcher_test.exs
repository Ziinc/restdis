defmodule SupaCacherReplicator.DispatcherTest do
  use ExUnit.Case, async: false

  alias SupaCacherReplicator.Dispatcher
  ...
  test "an insert on an unsubscribed dataset loads it and stores the row", %{
    ...
    refute SupaCacherReplicator.subscribed?(dataset)

    :ok = Dispatcher.dispatch(config, :insert, %{"id" => 7})
    Subscription.await_loaded(dataset)

    assert {:ok, %{"name" => "row-7"}} =
             SupaCacherReplicator.get(dataset.tenant_id, dataset.table, 7)
  end
  ...
  test "a delete removes the KV entry", %{dataset: dataset, config: config} do
    ...
    assert {:ok, _} = SupaCacherReplicator.get(dataset.tenant_id, dataset.table, 2)
  end
