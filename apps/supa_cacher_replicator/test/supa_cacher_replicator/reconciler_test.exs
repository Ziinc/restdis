defmodule SupaCacherReplicator.ReconcilerTest do
  use ExUnit.Case, async: false

  alias SupaCacherReplicator.Reconciler
  ...
  setup do
    ...
    Application.put_env(
      :supa_cacher_replicator,
      :dataset_source,
      {StubSource, :list_replicated, []}
    )

    on_exit(fn ->
      Application.put_env(:supa_cacher_replicator, :dataset_source, nil)
  ...
  test "reconciliation leaves zero stale entries across datasets", %{
    ...
    {:ok, _} = SupaCacherReplicator.subscribe(first)
    {:ok, _} = SupaCacherReplicator.subscribe(second)
    ...
    assert :miss = SupaCacherReplicator.get(first.tenant_id, first.table, 2)
    assert :miss = SupaCacherReplicator.get(first.tenant_id, first.table, 3)
    assert {:ok, %{"name" => "row-1"}} = SupaCacherReplicator.get(first.tenant_id, first.table, 1)
    assert :miss = SupaCacherReplicator.get(second.tenant_id, second.table, 1)

    assert {:ok, %{"name" => "changed"}} =
             SupaCacherReplicator.get(second.tenant_id, second.table, 2)
  end
  ...
  test "reconcile_all/0 staggers reconciliation of every dataset", %{first: first, second: second} do
    ...
    :ok = SupaCacherReplicator.reconcile_all()

    assert eventually(fn ->
             match?({:ok, _}, SupaCacherReplicator.get(first.tenant_id, first.table, 1)) and
               match?({:ok, _}, SupaCacherReplicator.get(second.tenant_id, second.table, 1))
           end)
  end
  ...
  test "reconcile_all/0 without a dataset source is a no-op" do
    ...
    assert :ok = SupaCacherReplicator.reconcile_all()
