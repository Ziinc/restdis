defmodule SupaCacherReplicator.Reconciler do
  @moduledoc """
  ...
  alias SupaCacherReplicator.Subscription.Supervisor, as: SubscriptionSupervisor
  ...
  defp reconcile(%Dataset{} = dataset) do
    case SubscriptionSupervisor.ensure_started(dataset) do
      ...
      {:error, reason} ->
        Logger.warning(
          "[SupaCacherReplicator] reconcile of #{dataset.table} failed: #{inspect(reason)}"
        )
    end
  end

  defp list_datasets do
    case Application.get_env(:supa_cacher_replicator, :dataset_source) do
  ...
  defp stagger_ms do
    Application.get_env(:supa_cacher_replicator, :reconcile_stagger_ms, @default_stagger_ms)
  end
end
