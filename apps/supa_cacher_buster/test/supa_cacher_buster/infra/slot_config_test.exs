defmodule SupaCacherBuster.Infra.SlotConfigTest do
  use ExUnit.Case, async: false

  alias SupaCacherBuster.Infra.SlotConfig

  setup do
    original = Application.get_env(:supa_cacher_buster, :replication_connection)
    on_exit(fn -> Application.put_env(:supa_cacher_buster, :replication_connection, original) end)
    :ok
  end

  defp put_conn(opts), do: Application.put_env(:supa_cacher_buster, :replication_connection, opts)
  ...
  test "a :url option is expanded into connection options" do
    put_conn(url: "ecto://alice:s3cret@db.internal:5433/supacacher", pool_size: 1)
  ...
  test "percent-encoded credentials are decoded" do
    put_conn(url: "postgres://a%40b:p%40ss@db.internal/supacacher")
  ...
  test "explicit options win over the url" do
    put_conn(url: "postgres://db.internal/supacacher", hostname: "override")
