defmodule SupaCacherBuster.WAL.RelationCacheTest do
  ...
  test "update/5 then lookup/2 returns the relation" do
    cols = [%{name: "id", flags: 1}, %{name: "name", flags: 0}]
    cache = RelationCache.new() |> RelationCache.update(100, "public", "products", cols)

    assert {:ok, %{schema: "public", table: "products", columns: ^cols}} =
             RelationCache.lookup(cache, 100)
  end

  test "update/5 overwrites existing OID" do
    cols1 = [%{name: "id", flags: 1}]
    cols2 = [%{name: "id", flags: 1}, %{name: "price", flags: 0}]

    cache =
      RelationCache.new()
      |> RelationCache.update(1, "public", "products", cols1)
      |> RelationCache.update(1, "public", "products", cols2)
  ...
  test "multiple OIDs coexist" do
    cache =
      RelationCache.new()
      |> RelationCache.update(1, "public", "products", [])
      |> RelationCache.update(2, "public", "orders", [])
