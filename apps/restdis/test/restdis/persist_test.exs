defmodule Restdis.Cache.PersistTest do
  ...
  test "cap enforcement: 3rd persist put returns error and count stays at cap", %{
    ...
    assert :ok = Restdis.Cache.put(tenant_id, key1, "v1", persist: true, persist_cap: 2)
    assert :ok = Restdis.Cache.put(tenant_id, key2, "v2", persist: true, persist_cap: 2)

    assert {:error, :persist_cap} =
             Restdis.Cache.put(tenant_id, key3, "v3", persist: true, persist_cap: 2)

    assert 2 = Restdis.Cache.persist_count(tenant_id)
  end

  test "set_persist false→true increments count", %{tenant_id: tenant_id} do
    key = Key.build(:table, "products", %{})
    assert :ok = Restdis.Cache.put(tenant_id, key, "val")
  ...
  test "set_persist true→false decrements count", %{tenant_id: tenant_id} do
    key = Key.build(:table, "products", %{})
    assert :ok = Restdis.Cache.put(tenant_id, key, "val", persist: true)
    assert 1 = Restdis.Cache.persist_count(tenant_id)
  ...
  test "set_persist returns not_found for absent key", %{tenant_id: tenant_id} do
    ...
    assert {:error, :not_found} = Restdis.Cache.set_persist(tenant_id, key, true)
  end
  ...
  test "persist_count is rebuilt from disk after supervisor restart", %{tenant_id: tenant_id} do
    ...
    assert :ok = Restdis.Cache.put(tenant_id, key1, "v1", persist: true)
    assert :ok = Restdis.Cache.put(tenant_id, key2, "v2", persist: true)
