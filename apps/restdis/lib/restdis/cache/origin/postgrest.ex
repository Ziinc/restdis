defmodule Restdis.Cache.Origin.PostgREST do
  ...
  path, so it cannot look up the original query string (see
  `RestdisServer.QueryStore`) or resolve which tenant's PostgREST to call.
  Phase 2 returns `:error` here; the server's `PGRST.QUERY` command handler
  performs the fetch-and-cache cycle using the full path string and tenant id
  available at command parse time, via `RestdisServer.PostgREST.Fetcher`.
  """
