defmodule RestdisReplicator.Subscription do
  @moduledoc """
  ...
  alias RestdisReplicator.Origin
  ...
  def handle_continue(:initial_load, state) do
    case load_all(state.dataset) do
      {:ok, rows} ->
        ...
        :telemetry.execute(
          [:restdis_replicator, :subscription, :loaded],
      ...
      {:error, reason} ->
        Logger.warning(
          "[RestdisReplicator] initial load of #{state.dataset.table} failed: #{inspect(reason)}"
        )
  ...
  def handle_call(:reconcile, _from, state) do
    case load_all(state.dataset) do
      {:ok, rows} ->
        ...
        :telemetry.execute(
          [:restdis_replicator, :reconcile, :completed],
  ...
  defp do_refresh_row(state, pk) do
    case Origin.fetch_row(state.dataset, pk) do
      ...
      {:error, reason} ->
        Logger.warning(
          "[RestdisReplicator] refresh of #{state.dataset.table}:#{pk} failed: #{inspect(reason)}"
        )
  ...
  defp via(dataset) do
    {:via, Registry, {RestdisReplicator.Registry, Dataset.registry_key(dataset)}}
  end

  defp page_size do
    Application.get_env(:restdis_replicator, :page_size, @default_page_size)
  end

  defp page_delay_ms do
    Application.get_env(:restdis_replicator, :page_delay_ms, @default_page_delay_ms)
  end
end
